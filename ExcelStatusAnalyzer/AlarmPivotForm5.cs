...
{
    public partial class AlarmPivotForm5 : Form
    {
        ...
        {
            Text = "Alarm Pivot Form 5 (모델별 합산: APAMA/APTURA)";
        ...
        private DataTable BuildModelSummary(XLWorkbook wb, int fromSheetIndex, int toSheetIndex, HashSet<string> whitelist)
        {
                ...
                .ThenBy(x => x.Key, StringComparer.OrdinalIgnoreCase))
            {
                ...
                row[ColMin] = Math.Round(totalMin, 0);
                row[ColMinPerCount] = Math.Round(minPer, 1);
                dt.Rows.Add(row);
            }
        ...
        // Copy (헤더 없이, 버튼으로만)
        // =========================
        private void CopyCurrentModelSummary()
        {
